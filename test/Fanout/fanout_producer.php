...
try {
     ...
     */
    $channel->queue_bind($queue_name1, $exchange);
    $channel->queue_bind($queue_name2, $exchange);


    // 将消息发布到交换机. 执行这两个推送，会往两个队列推送消息。 因为该交换机绑定了两个队列
    $channel->basic_publish(new AMQPMessage('rabbit message'), $exchange);  
    $channel->basic_publish(new AMQPMessage('rabbit.child message'), $exchange);
