...
try {
    ...
    $channel->queue_bind($queue_name2, $exchange, $routing_key1);
    $channel->queue_bind($queue_name3, $exchange, $routing_key2);
    $channel->queue_bind($queue_name4, $exchange, $routing_key3);

    // 将消息发布到交换机, 路由key1
    $channel->basic_publish(new AMQPMessage('k1 message'), $exchange, $routing_key1);
    $channel->basic_publish(new AMQPMessage('k1 message'), $exchange, $routing_key1);
    $channel->basic_publish(new AMQPMessage('k2 message'), $exchange, $routing_key2);
    $channel->basic_publish(new AMQPMessage('k3 message'), $exchange, $routing_key3);
